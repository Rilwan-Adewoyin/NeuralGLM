...
class VAEGAN(nn.Module):
    ...
    def __init__(self,
        ...
        self.grouped_conv2d_reduce_time = torch.nn.Conv2d( 6*4, 6, (1,1), groups=6 )
    ...
    def forward(self, variable_fields, constant_fields, image, noise=None, mask=None ): #ensemble_size=1
        
        image, variable_fields, constant_fields, z_mean, z_logvars = self.generator(variable_fields, constant_fields, noise)
        score = self.discriminator(image, variable_fields, constant_fields, mask )

        
        # return li_score, li_enc_output, li_image, li_z_mean, li_z_logvars #(b, 1)
        return score , image, z_mean, z_logvars
    
    def generator(self, variable_fields, constant_fields, noise=None ): #ensemble_size=1


        b, h, w = constant_fields.shape 
        _, _, _, d = variable_fields.shape

        #grouped convolution to reduce time dimension in variable fields
        variable_fields = einops.rearrange( variable_fields, '(b t) h w d ->b (d t) h w', t=4)
        ...
        z_mean, z_logvars = self.encoder(variable_fields, constant_fields)      

        noise = noise if noise is not None else torch.randn( (variable_fields.shape[0],
                                                                self.latent_variables,
                                                                *variable_fields.shape[2:]),
                                                            device=z_logvars.device)
            
        image = self.decoder(z_mean, z_logvars, constant_fields, noise )
        
        return image, variable_fields, constant_fields, z_mean, z_logvars
    ...
    def parse_model_args(parent_parser):
        ...
        model_parser.add_argument("--filters_disc", default=332,type=int)
...
class VAEGAN_encoder(nn.Module):
    ...
    def forward(self, variable_fields, constant_fields):
        ...
        z_mean = self.conv2d_mean(x)
        z_logvars = self.conv2d_logvars(x)
                           
        return z_mean, z_logvars
...
class VAEGAN_discriminator(nn.Module):
    def __init__(self,
        ...
        # block_outp_channels = [filters_disc*i for i in range(1, 1+num_resid_block_field) ]
        ...
        self.ds_resid_block_img = nn.Sequential(
            ...
            nn.ReLU(),
            ResidualBlock(block_outp_channels[0],block_inp_channels[1] ,conv_size, stride, relu_alpha, norm, dropout_rate, padding_mode, forceconv),
            
            nn.Conv2d(block_inp_channels[1], block_outp_channels[1],  (1,1), (1,1), padding='valid'),
            nn.ReLU(),
            ResidualBlock( block_outp_channels[1], block_outp_channels[1], conv_size, stride, relu_alpha, norm, dropout_rate, padding_mode, forceconv)            
        )
        
        # Residual block for output
        self.outp_residual_block = nn.Sequential(
            ResidualBlock(block_outp_channels[-1]*2, filters_disc, conv_size, stride, relu_alpha, norm, dropout_rate, padding_mode, forceconv),
            ResidualBlock(filters_disc, filters_disc, conv_size, stride, relu_alpha, norm, dropout_rate, padding_mode, forceconv)
        )
...
class ResidualBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        x_resid = self.highway_block(x) #make sure it operates on -3 channels dim
